/* 
  Safe and Sound - Capital City (TRUMPET SECTION)                              
...
/* 
  Safe and Sound - Capital City (BASS SECTION)                                
...
// sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
// there are two values per note (pitch and duration), so for each note there are four bytes
int notes = sizeof(bassMelody) / sizeof(bassMelody[0]) / 2;

// this calculates the duration of a whole note in ms (60s/tempo)*4 beats (milisonds to bpm)
int wholenote = (60000 * 4) / tempo;
...
while (digitalRead(7) == LOW)  {
  delay(10); 
    ...
    for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
      ...
      // Plays notes for 90% of the duration, and gives 10% leeway (this resolves the resonance of the piazza buzzer
      tone(bassMelodybuzzer, bassMelody[thisNote], noteDuration * 0.9);
      // Wait for the specify the duration of the note before playing the next note.
      delay(noteDuration);

      // stop the waveform generation before the next note.
      noTone(bassMelodybuzzer);
    }
  }
